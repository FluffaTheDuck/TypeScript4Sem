...
const createTable = (people) => {
    let tableHtml = "<table><thead><tr><td>#</td><td>Name</td><td>Age</td><td>Gender</td></tr></thead><tbody>";
    people.map((person, index) => (tableHtml += `<tr><td>${index + 1}</td><td>${person.name}</td><td>${person.age}</td><td>${person.gender}</td></tr>`));
